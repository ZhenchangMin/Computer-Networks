...
| 延迟类型                                 | 定义                                                                  | 公式                                   | 影响因素     |
| ---------------------------------------- | --------------------------------------------------------------------- | -------------------------------------- | ------------ |
| **Transmission delay（传输时延）** | 把包的所有bit推入链路需要多长时间和设备调制能力相关             | 包大小L / 链路带宽R                    | 链路带宽     |
| **Propagation delay（传播时延）**  | 一个bit从链路一端传到另一端的时间速度是在光速量级的，和距离相关 | 链路长度d / 传播速度v（≈3×10⁸ m/s） | 链路物理长度 |
