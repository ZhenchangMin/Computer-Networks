...
#### 一些有趣的情况
第一种情况，A向B发送一个序号为92的报文段，有8byte的数据，所以等待一个确认号为100的ACK
但是假设ACK丢失，那么超时事件发生，A重传序号为92的报文段
对于B，他收到一个包含收到过的数据的报文段，B会丢弃这个报文段，并且发送一个确认号为100的ACK

第二种情况，如果A连续发送了两个报文段，序号是92，包含8byte数据和序号为100，包含20byte数据
那么B会先后发送两个ACK
加入超时事件发生，A重传序号为92的报文段
这个时候两个ACK到达，那么只要100的ACK在新的超时发生之前到达，第二个报文就不会被重传

第三种情况，假如在超时之前，92的ACK丢失，100的ACK到达了，那么就不会重传
因为是累积确认，只有当<120的所有字节都收到的时候，才会发送确认号为120的ACK

#### 超时间隔加倍
每次超时事件发生，TCP重传最小序号的还未被确认的报文段，然后把下一次超时的时间间隔加倍，直到收到一个ACK为止
所以超时间隔会在每次重传之后呈现指数型增长
当然，每次遇到另外两个事件（从上层应用程序接收数据和收到一个ACK）都会把超时间隔重置为EstimatedRTT+4*devRTT
拥塞的时候，如果一直重传分组，会让拥塞更严重。因此把超时间隔加倍可以让网络有时间恢复，避免过度拥塞

#### 快速重传
超时周期可能比较长，报文段丢失的时候，可能增加端到端时延
由于TCP不使用否定确认，所以通过冗余ACK来检测丢失的报文段
如果收到了比期望的序号大的报文段，就说明数据流有间隔，报文段有丢失
![1781941092102](image/lec3TransferLayer/1781941092102.png)
第一行就是正常的理想情况，之所以延迟发送是因为后面可能马上有跟着的报文段。这样可以减少一个ACK开销
第二行是紧跟第一行的，如果已经有第一行那样的延迟ACK，然后期望的报文段紧跟着正确接受，那么马上发送一个ACK，不让发送方等太久，否则可能会超时导致不必要的重传
第三行就是提前到达的报文段，发送方会收到一个冗余ACK，说明有报文段丢失了
第四行就是有能填补数据流空缺的到达了

一旦收到了3个冗余ACK，发送方就可以立即重传丢失的报文段，而不必等待超时事件发生，这就是**快速重传**
快速是因为不用等超时事件发生，就进行重传，这样很快

#### 回退N步还是选择重传
TCP是一个GBN还是SR协议？
由于TCP的累积确认，如果对于1, 2...N的报文段，如果都正确抵达，但是n的ACK丢失了，那么GBN会重传n, n+1, n+2...N的报文段，而TCP至多重传报文段n。之所以是至多，是因为如果n+1的ACK在n超时之前到达，n甚至不会被重传

TCP实际上是**选择确认**的，也就是TCP的ACK报文段中有一个选项字段，叫做SACK（Selective ACK），用来指明接收方已经正确接收了哪些报文段
TCP的SACK选项允许接收方告诉发送方他已经正确接收了哪些报文段，这样发送方就可以仅重传丢失的报文段，而不是重传所有未确认的报文段

TCP因此更像是GBN和SR的混合体

### 流量控制
TCP是双工的，所以他两头都有一个接收缓存。如果某个应用程序从缓存里面读取数据很慢，而发送方法的数据又太多太快，就会导致**接收缓存溢出**。
TCP提供了**流量控制服务**来消除这个问题，确保发送方不会发送超过接收方缓存容量的数据
实际上是个速度匹配的服务，确保发送速率与读取速率相匹配。
流量控制和拥塞控制是不同的，尽管操作可能相似，但是他们是为了解决不同的问题而采取的措施
流量控制是由于接收缓存溢出，拥塞控制是由于IP网络的拥塞

发送方维护一个**接收窗口**的变量，这个变量就是说该接收方还有多少可用的缓存空间。
假设A给B发送一个大文件，B给该TCP连接分配了一个接收缓存，假设大小是RcvBuffer
定义如下变量：
- LastByteRead：上层应用程序从接收缓存中读取的最后一个字节的序号
- LastByteRcvd：到达的并且已经放入B的接收缓存中的最后一个字节的序号
TCP不允许缓存溢出，所以下式必须成立
$$
...
$$
![1781943715002](image/lec3TransferLayer/1781943715002.png)
主机B把当前的rwnd值放入ACK报文段的接收窗口字段中，发送给A
一开始的时候rwnd=RcvBuffer
主机A会跟踪两个变量：
- LastByteSent：发送的最后一个字节的序号
- LastByteAcked：被确认的最后一个字节的序号
这两个变量的差就是发送方已经发送但是还没有被确认的字节数
这两个的差不能超过rwnd，否则就会导致接收缓存溢出
$$
LastByteSent - LastByteAcked \leq rwnd
$$
rwnd=0的时候，B的缓存满了，而且由于A不再发送，B也不再发送rwnd新值，导致阻塞。
TCP要求rwnd=0时候A依然发生只有一个字节数据的报文段，这样就可以让B发送一个rwnd新值，告诉A接收缓存有空间了

UDP不提供流量控制

### TCP连接管理（建立和拆除）
#### TCP连接建立
- 第一步，客户端的TCP向服务器端的TCP发送一个**特殊的TCP报文段**。该报文段不包含任何应用层数据，SYN位被置为1，序号字段被设置为客户端的初始序号client_isn
- 第二步，服务器端的TCP收到这个报文段后，发送一个**特殊的TCP报文段**作为响应。该报文段不包含任何应用层数据，SYN位被置为1，序号字段被设置为服务器的初始序号server_isn，同时确认号字段被设置为client_isn+1 这个报文段被称为**SYNACK报文段**
- 第三步，收到SYNACK之后，客户主机发生一个报文段，确认号字段被设置为server_isn+1，SYN位被置为0
这就是TCP连接建立的**三次握手**过程

#### TCP连接拆除
客户和服务器进程都可以终止连接，我们假设是客户进程发起终止连接的请求
客户应用程序发出关闭连接命令，客户TCP发生一个特殊的TCP报文段，FIN位被置为1，序号字段被设置为客户的最后一个字节的序号+1
服务器TCP收到这个报文段后，发送一个确认报文段，ACK位被置为1，确认号字段被设置为客户的最后一个字节的序号+1
然后，服务器TCP发送他自己的FIN终止报文段。
![1782005343927](image/lec3TransferLayer/1782005343927.png)
这个图体现了客户TCP经历的一系列典型的TCP状态
注意这里等待30秒是确保服务器TCP收到客户TCP的对于服务器FIN的ACK报文段了，如果服务器TCP没有收到这个ACK报文段，那么他会重传FIN报文段，直到收到ACK为止
因此这个TIME_WAIT状态是必要的
![1782006583398](image/lec3TransferLayer/1782006583398.png)
假设是客户端开始拆除连接，这个就是服务器端的TCP状态图

如果主机接受了一个有端口为80的TCP SYN分组，但是该主机的端口80不接受连接，那么该主机就会发送一个特殊重置报文段，其中的RST位被置为1

### TCP拥塞控制
#### 经典TCP拥塞控制
拥塞控制是由于路由器缓存溢出而引起的，分组重传因此作为网络拥塞的征兆
TCP通过让每一个发送方感知网络拥塞程度来限制他向连接发送流量的速率

首先，我们看TCP如何限制发送方发送流量
发送方维护一个**拥塞窗口**变量cwnd
$$
...
为了关注拥塞控制我们假设TCP接收缓存足够大，于是数据量仅受限于cwnd
通过限制发送方中的未被确认的数据量，间接限制了发送速率
也就是每次RTT起始的时候，限制发送方至多发送cwnd字节的数据，所以发送速率大概就是cwnd/RTT byte/s

那么TCP发送方又是如何感知路径出现拥塞的呢？
TCP发送方的**丢包事件**定义为：超时或收到来自接收方的3个冗余ACK

假如没有出现丢包事件，TCP将正常收到未确认报文的ACK。如果ACK到达的慢，那么cwnd增加的就慢，如果ACK到达的快，那么cwnd增加的就快
所以TCP使用ACK来增加他的cwnd长度，是**自计时**的

TCP如何确定发送的速率？
- 一个丢失的报文段意味着网络拥塞，因此丢失报文段的时候应该降低TCP发送方的速率
- 一个ACK指示一切顺利，代表网络不拥塞，应该增加TCP发送方的速率
- 带宽检测。网络中没有明显的拥塞状态的指令，而是通过ACK和丢包事件来作为隐式信号

**TCP拥塞控制算法**
主要包括慢启动、拥塞避免、快速恢复三个部分，前两个强制，第三个是推荐的

##### 慢启动
一条TCP连接开始时，cwnd设置为一个MSS，所以初始发送速率是MSS/RTT
对于发送方而言，可用带宽远大于这个数字，所以在**慢启动**状态，cwnd的值从1个MSS开始，每次传输的报文段首次确认，就增加一个MSS
这样每次过了一个RTT的时候，cwnd的值就会翻倍，发送速率也就翻倍，所以发送速率起始慢，但是以指数速率增长

该增长何时停止？
首先，如果存在一个**超时**指示的丢包事件，cwnd就设置为1个MSS，慢启动重新开始，同时变量ssthresh（慢启动阈值）被设置为cwnd的一半
当达到或超过了ssthresh时，TCP进入**拥塞避免模式**，这就是这个阈值，慢启动结束
如果收到了3个**冗余ACK**，这个时候执行快速重传，并且进入**快速恢复模式**

##### 拥塞避免
进入拥塞避免状态，cwnd的值大概就是上次遇到拥塞时候的值的一半，所以TCP不再用翻倍的方法，而是更加保守地每次增加一个MSS。
也就是说每过一个RTT，cwnd增加1个MSS
这可以用以下方式完成：假设MSS是1460byte，现在cwnd是14600，也就是说每个RTT内发送10个报文段，那么每个报文段的ACK收到的时候就增加cwnd的1/10个MSS，这样在一个RTT内cwnd就增加了1个MSS

何时结束这样cwnd的线性增长？
出现超时的时候，与慢启动一样，cwnd被设置成1个MSS，ssthresh被设置成cwnd的一半，进入慢启动模式
3个冗余ACK的时候，cwnd减半，把ssthresh被设置成cwnd的一半，进入快速恢复模式

##### 快速恢复
在快速恢复中，对于引起进入快速恢复的缺失报文段，每次收到一个冗余ACK，cwnd增加1个MSS
最终，对于丢失报文段的ACK到达的时候，cwnd被设置为ssthresh的值，进入拥塞避免模式
同样的，如果超时，cwnd被设置为1个MSS，ssthresh被设置为cwnd的一半，进入慢启动模式
